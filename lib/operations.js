...
function runQuery({con, query, params=[]}){
...
const lowOperations = {
	run: async(query, params)=>{
		...
		const results = await runQuery({con, query, params})
		return results;
	},
	...
	insert: async(table='', data)=>{
		...
		const results = await runQuery({con, query, params: data})
		return results.insertId;
	},

	select: async({table='', fields=[], where="", params=[], orderby={}, additions=""} = {})=>{
		const con = await getDBCon();
		...
		const results = await runQuery({con, query, params: paramsAll})
		return results;
	},

	update: async({table='', fields={}, where="", params=[], additions=""} = {})=>{
		const con = await getDBCon();
	...
	delete: async({table='', where="", params=[], additions=""} = {})=>{
		const con = await getDBCon();
...
const highOperations = {
	get: async({table="", fields=[], where="", params=[], additions=""} = {})=>{
		let results = await lowOperations.select({table, fields, where, params, additions}, "LIMIT 1");
		if(results.length === 0){
			return null;
		}
		else{
	...
	doesExist: async({table='', where=''} = {})=>{
		let results = await highOperations.get({table, fields: ['id'], where});
		if(results === null){
			return(false);
		}
		else{
...
const createTables = async (dbSchema, tablesIgnored=[])=>{
	const con = await getDBCon();
	...
	await runQuery({con, query: tablesQueries});
